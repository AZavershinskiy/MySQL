...
-- Выборка данных по пользователю: ФИО, город проживания, ФОТО ПРОФИЛЯ
SELECT
    u.firstname,
    u.lastname, -- u = users
    p.hometown AS city, -- p - profile
    m.filename AS mediafile -- m = media
FROM users u
JOIN `profiles` p ON p.user_id = u.id -- получил p.hometown
JOIN media m ON p.photo_id = m.id;

-- Выборка данных по пользователю: ФИО, город проживания, ВСЕ ФАЙЛЫ
SELECT
    u.firstname,
    u.lastname, -- u = users
    p.hometown AS city, -- p - profile
...
FROM users u
JOIN `profiles` p ON p.user_id = u.id -- получил p.hometown
JOIN media m ON p.user_id = u.id
ORDER BY lastname;
